<?php
// 1. Start session to get user_id for the history
session_start();
// 2. THIS FIXES THE SCREENSHOT ERROR: Stops DB warnings from breaking the JSON
error_reporting(0); 
header('Content-Type: application/json');

// 3. Include the DB connection
include 'db.php'; 
...
if ($input && $apiKey) {
    // 4. Sanitize all inputs just like your registration page
    ...
    // --- FIND THE USER'S EMAIL & ID (The missing part in your 1111.php) ---
    $customerEmail = null;
    $user_id = 0; 
    ...
    if (isset($_SESSION['email'])) {
        ...
        // Search the database for the ID and Email if the session is lost
    ...
    // --- THE BIG FIX: SAVE TO THE ORDERS TABLE ---
    // This part was completely missing in your 1111.php file!
    ...
    // --- SEND THE PROFESSIONAL EMAIL (Keep your design) ---
    if ($customerEmail) {
        $rows = "";
        foreach($cart as $p) {
            $sub = $p['price'] * $p['qty'];
    ...
    // Always return success for the Telegram message to send
